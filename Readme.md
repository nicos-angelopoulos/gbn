...
pdf collecting all family heatmaps. These can be extremely useful for including into supplementary information of publications. During this step 
gates are all fitted for each family (files ending in *_gates_best.csv).
Continuing on the above example, we have:
...
gbn_gates_nets/1 can be used to produce visualisation of gated versions of the output networks. This predicate utilises the family gates from the last step.
The user can pass a number options to control wheher xor and not gates are included, and whether gates are to be simplified (see options for gbn_family_gates/5).
For our example: 


```
- gbn_gates_nets(dir('aml_min60-21.07.06')).
true.

?- ls('aml_min60-21.07.06').
% aml_min60.bn                aml_min60_fams/             aml_min60_fclr_gated.dot    aml_min60_fisher.dot        aml_min60_multi_prns.png    aml_min60.set
% aml_min60.dat               aml_min60_fclr.csv          aml_min60_fclr_gated.svg    aml_min60_fisher.svg        aml_min60_prns/             aml_min60_std_output.txt
% aml_min60.dot               aml_min60_fclr.dot          aml_min60_fclr.svg          aml_min60_gates_best.csv    aml_min60_scnti.txt         aml_min60.svg
true.

```

Finally, predicate gbn_svg_legend/1 adds legends to both types of networks generated by gbn: gated and narmal BNs. In the gated version the predicate
will only add legends for the gates used in the network so is able to adjust to the options used when generating the gated networks. (Gates not and xor can
be excluded if so wished by the user.) The predicate works by parsing the source of the SVG file trying to guess a location where the legend might fit and then
injecting SVG code into the file to create a new version which includes the appropriate legend.
The final step in our example is:

```
?- gbn_svg_legend(dir('aml_min60-21.07.06')).
true.

?- ls('aml_min60-21.07.06').
% aml_min60.bn                      aml_min60_fclr.dot                aml_min60_fclr.svg                aml_min60_multi_prns.png          aml_min60.svg
% aml_min60.dat                     aml_min60_fclr_gated.dot          aml_min60_fisher.dot              aml_min60_prns/                   
% aml_min60.dot                     aml_min60_fclr_gated_leg.svg      aml_min60_fisher_leg.svg          aml_min60_scnti.txt               
% aml_min60_fams/                   aml_min60_fclr_gated.svg          aml_min60_fisher.svg              aml_min60.set                     
% aml_min60_fclr.csv                aml_min60_fclr_leg.svg            aml_min60_gates_best.csv          aml_min60_std_output.txt          
true.
```
