...
gob_prob_name_settings( ProbName, _DBname, SetsF, Opts ) :-
     ...
     gob_prob_name_setting_adjmat( AdjMat, ProbName, AmF ),
     gob_stream_setting( Out, 'outputfile/adjacencymatrixfile', AmF ),
     options( palim(PaLim), Opts ),
     gob_stream_setting( Out, 'palim', PaLim ),
     findall( _, (member(setting(Key,Val),Opts),gob_stream_setting(Out,Key,Val)), _ ),
     close( Out ).

gob_prob_name_setting_adjmat( false, _, '' ) :- !.
gob_prob_name_setting_adjmat( true, ProbName, AmF ) :- 
     !,
     atomic_list_concat( [ProbName,'adjmat.txt'], '_', AmF ).
gob_prob_name_setting_adjmat( Else, _ProbName, Else ).
