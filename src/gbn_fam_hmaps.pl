...
gbn_fam_hmaps( Args ) :-
    ...
    os_sel( os_files, ext(dot), GoBn, dir(Dir) ),
...
gbn_fam_hmaps_dlists( Dir, Dlists, Opts, GoBnF ) :-
    ...
    % 26.05.26 Gobnilp no longer produces the .bn files, we are going for .dot
    % gbn_term( GoBnP, GoBnPrv, _Ado ),
    ( os_ext(bn,BnStemP,GoBnP) -> ext(dot,BnStemP,BnDotP); BnDotP = GoBnP ),
    dot_file_bn_fams( BnDotP, GoBnPrv ),
